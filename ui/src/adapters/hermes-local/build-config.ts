...
export function buildHermesLocalConfig(v: CreateConfigValues): Record<string, unknown> {
  ...
  config.model = v.model || DEFAULT_HERMES_LOCAL_MODEL;
  ...
  if (v.command) config.hermesCommand = v.command;

  const extraArgs = v.extraArgs ? parseCommaArgs(v.extraArgs) : [];
  if (v.thinkingEffort) {
    extraArgs.push("--reasoning-effort", v.thinkingEffort);
  }
