...
def main() -> int:
    ...
    board_review_json_path = Path(social_target.get("board_review_json_path") or Path(board_review_path).with_suffix(".json")).expanduser().resolve()
    approval_packet_json_path = Path(social_target.get("approval_packet_json_path") or Path(approval_packet_path).with_suffix(".json")).expanduser().resolve()

    # Validate upstream freshness: check board_review and approval_packet outputs independently
    require_fresh_upstream_stage_outputs(
        manifest, "board_review", {"board review json": board_review_json_path},
        context="Connector runbooks", force=args.force,
    )
    require_fresh_upstream_stage_outputs(
    ...
    write_pair(
        ...
        markdown_body="\n".join(
            [
                ...
                "3. Verify the build status, deployment logs, environment variables, and domain routing.",
    ...
    finalize_stage_outputs(
        ...
        manifest,
        status_updates={
            "riverside_runbook": "ready",
            "vercel_runbook": "ready",
            "fable_runbook": "ready",
        },
        provenance_updates={
            "riverside_runbook": (shared_inputs, {"riverside_runbook": riverside_json}),
            "vercel_runbook": (shared_inputs, {"vercel_runbook": vercel_json}),
            "fable_runbook": (shared_inputs, {"fable_runbook": fable_json}),
        },
        generated_at=generated_at,
    )
