...
def main() -> int:
    ...
    # Validate upstream freshness for all consumed artifacts
    require_fresh_inputs_from_stages(
        manifest,
        stages={
    ...
    instagram_path = Path(social_target["instagram_reel_path"]).expanduser().resolve()
    facebook_path = Path(social_target["facebook_post_path"]).expanduser().resolve()
    tiktok_path = Path(social_target["tiktok_post_path"]).expanduser().resolve()
...
"""
    ...
    finalize_stage_outputs(
        manifest_path,
        manifest,
        status_updates={"social_drafts": "ready"},
        target_sections={
            "social_poster": {
                "instagram_reel_path": str(instagram_path),
                "facebook_post_path": str(facebook_path),
                "tiktok_post_path": str(tiktok_path),
            },
        },
        provenance_updates={
            "social_drafts": (
