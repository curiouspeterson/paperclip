...
from pipeline_common import (
    ...
    require_numeric_transcript_segments,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--llm-api-key",
        default=os.environ.get("RU_CLIP_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY"),
    ...
    parser.add_argument(
        "--llm-timeout-seconds",
        type=int,
        default=int(os.environ.get("RU_LLM_TIMEOUT_SECONDS", "20")),
...
def load_transcript_segments(manifest: dict, transcript_path: Path) -> list[dict]:
    ...
    raise SystemExit(
        "Clip candidates require numeric timestamps from structured transcript segments. "
        "transcript_segments_path or transcript_json_path with valid start/end times must be provided."
    )
...
def rerank_with_llm(
    ...
    *,
    ...
    parsed = chat_json(
        ...
        api_key=api_key,
        model=model,
        system_prompt=CLIP_SELECTION_SYSTEM_PROMPT,
        user_payload=prompt,
...
def main() -> int:
    ...
    # Validate upstream transcript freshness using recorded outputs
    require_fresh_upstream_stage_outputs(
        manifest,
        "transcript",
        {"transcript": transcript_path},
        context="Clip candidates",
        force=args.force,
    )
    ...
    require_numeric_transcript_segments(segments, context="Clip candidates")
    ...
    # Abort before writing if any candidate lacks numeric timestamps
    require_numeric_clip_timestamps(clip_json_payload["clip_candidates"], context="Clip candidates serialization")
    ...
    finalize_stage_outputs(
        ...
        manifest,
        status_updates={
            "clip_candidates": "ready",
            "quote_candidates": "ready",
        },
        provenance_updates={
            "clip_candidates": (
                {"transcript": transcript_path},
                {"candidates_md": clip_candidates_path, "candidates_json": clip_candidates_json_path},
            ),
            "quote_candidates": (
                {"transcript": transcript_path},
                {"quotes_md": quote_candidates_path},
            ),
        },
    )
