...
def main() -> int:
    ...
    newsletter_draft_json_path = Path(
        newsletter_target.get("draft_json_path") or Path(newsletter_target["draft_path"]).with_suffix(".json")
    ).expanduser().resolve()
    ...
    quote_candidates_path = Path(clip_target["quote_candidates_path"]).expanduser().resolve()

    require_fresh_inputs_from_stages(
        manifest,
        stages={
            "approval_packet": {"approval packet json": approval_packet_json_path},
            "newsletter_draft": {"newsletter draft json": newsletter_draft_json_path},
            "clip_candidates": {"candidates_json": clip_candidates_json_path},
            "rendered_clips": {"rendered_json": rendered_clips_json_path},
            "quote_candidates": {"quotes_md": quote_candidates_path},
            "quote_cards": {"cards_json": quote_cards_json_path},
            "social_drafts": {
                "instagram": Path(social_target["instagram_reel_path"]).expanduser().resolve(),
                "facebook": Path(social_target["facebook_post_path"]).expanduser().resolve(),
                "tiktok": Path(social_target["tiktok_post_path"]).expanduser().resolve(),
            },
        },
        context="Board review generation",
        force=args.force,
    )
    ...
    # Record unified provenance for board_review stage
    generated_artifacts = {
        "board review": board_review_path,
        "board review json": board_review_json_path,
    }
    record_stage_provenance(
        manifest, "board_review", upstream_artifacts=artifact_paths, generated_artifacts=generated_artifacts
    )
    ...
    atomic_save_json(resolve_path(manifest_path), manifest)
