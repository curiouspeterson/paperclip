...
from pipeline_common import load_json, save_json, atomic_save_json, require_ready_statuses, record_stage_provenance
...
def canonical_transcript_paths(manifest_source: dict, transcript_path: Path) -> dict[str, Path]:
    """Build transcript output paths from manifest source configuration.

    Args:
        manifest_source: The manifest["source"] dict with transcript path fields
        transcript_path: The main transcript path (manifest["source"]["transcript_path"])

    Returns:
        Dict of transcript output paths keyed by format (txt, json, segments, diarization, srt, vtt, tsv)
    """
    return {
        "txt": transcript_path,
        "json": Path(manifest_source.get("transcript_json_path", transcript_path.with_suffix(".json"))).expanduser().resolve(),
        "segments": Path(
            manifest_source.get("transcript_segments_path", transcript_path.with_suffix(".segments.json"))
        ).expanduser().resolve(),
        "diarization": Path(
            manifest_source.get("transcript_diarization_path", transcript_path.with_suffix(".diarization.json"))
        ).expanduser().resolve(),
        "srt": Path(manifest_source.get("transcript_srt_path", transcript_path.with_suffix(".srt"))).expanduser().resolve(),
        "vtt": Path(manifest_source.get("transcript_vtt_path", transcript_path.with_suffix(".vtt"))).expanduser().resolve(),
        "tsv": Path(manifest_source.get("transcript_tsv_path", transcript_path.with_suffix(".tsv"))).expanduser().resolve(),
    }


def normalize_transcript_json(payload: dict, *, manifest: dict, model: str, language: str, diarization_payload: dict | None = None) -> dict:
    """Normalize Whisper JSON output into canonical transcript segments.

    Args:
        payload: Whisper JSON output dict
        manifest: Episode manifest dict (replaces legacy metadata parameter)
        model: Resolved model name used for transcription
        language: Language code
        diarization_payload: Optional diarization output from pyannote

    Returns:
        Normalized transcript dict with segments and metadata
    """
...
def main() -> int:
    ...
    require_ready_statuses(manifest, ("manifest",), context="Transcript")
    ...
    transcript_paths = canonical_transcript_paths(manifest["source"], transcript_path)
    ...
    if all(path.exists() for path in transcript_paths.values()) and not args.force:
        # Certify the existing outputs: record provenance so downstream freshness checks
        # have a certified baseline even when transcription is skipped.
        output_provenance: dict[str, Path] = {
            "transcript": transcript_paths["txt"],
            "transcript_json": transcript_paths["json"],
            "transcript_segments": transcript_paths["segments"],
        }
        for key in ("srt", "vtt", "tsv"):
            if transcript_paths[key].exists():
                output_provenance[f"transcript_{key}"] = transcript_paths[key]
        diarization_path = transcript_paths.get("diarization")
        if diarization_path and diarization_path.exists():
            output_provenance["transcript_diarization"] = diarization_path
        source_path_for_prov = Path(manifest["source"]["media_path"]).expanduser().resolve()
        record_stage_provenance(
            manifest,
            "transcript",
            upstream_artifacts={"source": source_path_for_prov},
            generated_artifacts=output_provenance,
        )
    ...
    # Mark transcript as running in manifest
    manifest.setdefault("status", {})["transcript"] = "running"
    manifest.setdefault("artifacts", {}).setdefault("transcript", {})
    manifest["artifacts"]["transcript"]["started_at"] = datetime.now(UTC).isoformat()
    manifest["artifacts"]["transcript"]["backend"] = backend
    manifest["artifacts"]["transcript"]["preset"] = args.preset
    manifest["artifacts"]["transcript"]["model"] = resolve_model(args, backend)
    manifest["artifacts"]["transcript"]["word_timestamps"] = parse_bool(args.word_timestamps, default=True)
    manifest["artifacts"]["transcript"]["decoder"] = {
        **resolve_decoder_options(args),
        "initial_prompt": bool(args.initial_prompt),
    }
    manifest["artifacts"]["transcript"]["diarization_backend"] = diarization_backend
    manifest["artifacts"]["transcript"]["diarization_status"] = "running" if diarization_backend != "none" else "skipped"
    manifest["updated_at"] = datetime.now(UTC).isoformat()
    atomic_save_json(manifest_path, manifest)
    ...
    with tempfile.TemporaryDirectory(prefix="ru-whisper-") as tmp_dir:
        ...
        try:
            ...
            manifest.setdefault("status", {})["transcript"] = "failed"
            manifest.setdefault("artifacts", {}).setdefault("transcript", {})
            manifest["artifacts"]["transcript"]["failed_at"] = datetime.now(UTC).isoformat()
            manifest["artifacts"]["transcript"]["error"] = str(exc)[:1200]
            manifest["updated_at"] = datetime.now(UTC).isoformat()
            atomic_save_json(manifest_path, manifest)
        ...
        if not generated_paths["txt"].exists():
            ...
            manifest.setdefault("artifacts", {}).setdefault("transcript", {})
            manifest["artifacts"]["transcript"]["failed_at"] = datetime.now(UTC).isoformat()
            manifest["artifacts"]["transcript"]["error"] = "Transcriber completed without producing a transcript file"
            manifest["updated_at"] = datetime.now(UTC).isoformat()
            atomic_save_json(manifest_path, manifest)
        ...
        if generated_paths["json"].exists():
            ...
            if diarization_backend == "pyannote":
                try:
                    ...
                    manifest.setdefault("status", {})["diarization"] = "ready"
                    manifest.setdefault("artifacts", {}).setdefault("transcript", {})
                    manifest["artifacts"]["transcript"]["diarization_generated_at"] = datetime.now(UTC).isoformat()
                    manifest["artifacts"]["transcript"].pop("diarization_error", None)
                except Exception as exc:
                    manifest.setdefault("status", {})["diarization"] = "failed"
                    manifest.setdefault("artifacts", {}).setdefault("transcript", {})
                    manifest["artifacts"]["transcript"]["diarization_error"] = str(exc)[:1200]
            else:
                manifest.setdefault("status", {})["diarization"] = "skipped"
            save_json(
    ...
    # Mark transcript as ready in manifest
    manifest.setdefault("status", {})["transcript"] = "ready"
    manifest.setdefault("artifacts", {}).setdefault("transcript", {})
    manifest["artifacts"]["transcript"]["generated_at"] = datetime.now(UTC).isoformat()
    manifest["artifacts"]["transcript"]["backend"] = backend
    manifest["artifacts"]["transcript"]["model"] = resolved_model
    # Clear any previous error state
    manifest["artifacts"]["transcript"].pop("error", None)
    # Record provenance: input = source media, outputs = all produced transcript files
    source_path = Path(manifest["source"]["media_path"]).expanduser().resolve()
    output_provenance: dict[str, Path] = {"transcript": transcript_paths["txt"], "transcript_json": transcript_paths["json"], "transcript_segments": transcript_paths["segments"]}
    for key in ("srt", "vtt", "tsv"):
        if transcript_paths[key].exists():
            output_provenance[f"transcript_{key}"] = transcript_paths[key]
    diarization_path = transcript_paths.get("diarization")
