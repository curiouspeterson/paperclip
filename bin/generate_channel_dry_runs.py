...
def main() -> int:
    ...
    approval_packet_json_path = Path(social_target["approval_packet_json_path"]).expanduser().resolve()
    rendered_clips_path = Path(clip_target["rendered_clips_path"]).expanduser().resolve()
    rendered_clips_json_path = Path(clip_target["rendered_clips_json_path"]).expanduser().resolve()
    quote_cards_path = Path(clip_target["quote_cards_path"]).expanduser().resolve()
    quote_cards_json_path = Path(clip_target["quote_cards_json_path"]).expanduser().resolve()
    board_review_json_path = Path(social_target.get("board_review_json_path") or board_review_path.with_suffix(".json")).expanduser().resolve()

    require_existing_paths(
        {
            "board review": board_review_path,
            ...
            "rendered clips": rendered_clips_path,
            "rendered clips json": rendered_clips_json_path,
            "quote-card specs": quote_cards_path,
            "quote-card json": quote_cards_json_path,
    ...
    # Validate upstream freshness: check each upstream stage's recorded outputs independently
    newsletter_draft_json_path = Path(newsletter_target["draft_json_path"]).expanduser().resolve()
    require_fresh_upstream_stage_outputs(
        manifest, "board_review", {"board review json": board_review_json_path},
        context="Channel dry runs", force=args.force,
    )
    require_fresh_upstream_stage_outputs(
        manifest, "approval_packet", {"approval packet json": approval_packet_json_path},
        context="Channel dry runs", force=args.force,
    )
    require_fresh_upstream_stage_outputs(
        manifest, "newsletter_draft", {"newsletter draft json": newsletter_draft_json_path},
        context="Channel dry runs", force=args.force,
    )
    require_fresh_upstream_stage_outputs(
        manifest, "rendered_clips", {"rendered_json": rendered_clips_json_path},
        context="Channel dry runs", force=args.force,
    )
    require_fresh_upstream_stage_outputs(
        manifest, "quote_cards", {"cards_json": quote_cards_json_path},
        context="Channel dry runs", force=args.force,
    )

    approval_packet = load_json(approval_packet_json_path)
...
"""
    ...
    dry_run_inputs = {
        "board review json": board_review_json_path,
        "approval packet json": approval_packet_json_path,
    ...
    finalize_stage_outputs(
        ...
        provenance_updates={
            "instagram_dry_run": (dry_run_inputs, {"instagram_dry_run": instagram_json_path}),
            "mailchimp_dry_run": (dry_run_inputs, {"mailchimp_dry_run": mailchimp_json_path}),
        },
        generated_at=generated_at,
    )
