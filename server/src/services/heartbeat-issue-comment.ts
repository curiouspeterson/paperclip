...
const HERMES_TOOL_ONLY_EXIT_MESSAGE =
...
function buildIntro(status: string, summarizedResult: Record<string, unknown> | null) {
  ...
  const hasHermesToolOnlyExit =
    summary === HERMES_TOOL_ONLY_EXIT_MESSAGE
    || message === HERMES_TOOL_ONLY_EXIT_MESSAGE
    || result === HERMES_TOOL_ONLY_EXIT_MESSAGE;

  if (hasHermesToolOnlyExit && status === "succeeded") {
    return "Run completed, but Hermes exited after tool calls without a final assistant completion.";
  }
...
export function buildHeartbeatRunIssueComment(input: {
  ...
  const lines = [buildIntro(input.status, summarizedResult), ""];
