...
Idempotent if you already own the task. If your previous run crashed while you still own an `in_progress` task, repeat the same checkout request with the new `X-Paperclip-Run-Id`; the server will adopt the stale lock when it is safe to do so. **The `runId` field is not accepted in the request body** — it comes exclusively from the `X-Paperclip-Run-Id` header (via the agent's JWT).
