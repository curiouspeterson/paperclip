...
export async function listSkillEntriesFromDirectories(
  ...
  for (const rawDir of dirs) {
    ...
    if (await hasSkillManifest(candidate)) {
      const runtimeName = path.basename(candidate);
      const key = `${runtimeName}:${candidate}`;
      if (!seen.has(key)) {
        seen.add(key);
        entries.push({ key, runtimeName, source: candidate });
      }
      continue;
    }
    ...
    for (const child of childEntries) {
      ...
      seen.add(key);
      entries.push({ key, runtimeName: child.name, source });
    }
  }
