...
export async function ensureCodexSkillsInjected(
  ...
  const runtimeEntries = options.skillsEntries ?? await listPaperclipSkillEntries(__moduleDir);
  const extraEntries = options.additionalSkillDirs?.length
    ? await listSkillEntriesFromDirectories(options.additionalSkillDirs)
    : [];
  const allSkillsEntries = [...runtimeEntries];
  const seen = new Set(allSkillsEntries.map((entry) => `${entry.key}:${entry.source}`));
  for (const entry of extraEntries) {
    const entryKey = `${entry.key}:${entry.source}`;
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const externalSkillEntries = externalSkillDirs.length
    ? await listSkillEntriesFromDirectories(externalSkillDirs)
    : [];
  const desiredSkillNames = resolveCodexDesiredSkillNames(config, [
    ...codexSkillEntries,
    ...externalSkillEntries,
  ]);
  ...
  await ensureCodexSkillsInjected(
    onLog,
    {
      ...
      skillsEntries: [...codexSkillEntries, ...externalSkillEntries],
      desiredSkillNames,
    },
  );
