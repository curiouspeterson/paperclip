...
async function ensureOpenCodeSkillsInjectedFromDirectories(
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[paperclip] Injected OpenCode external skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[paperclip] Failed to inject OpenCode external skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  await ensureOpenCodeSkillsInjected(
    onLog,
    [...openCodeSkillEntries, ...externalSkillEntries],
