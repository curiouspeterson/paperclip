...
If RU_HOMEPAGE_DEPLOY=1 is set, the homepage update step also triggers a Vercel
production deploy from the static site directory after the homepage data file is refreshed.
EOF
}
...
if [[ -z "$METADATA_PATH" || -z "$MANIFEST_PATH" ]]; then
  echo "Failed to resolve metadata or manifest path from detect step" >&2
  exit 1
fi

if [[ -n "$SOURCE_PUBLIC_URL" || -n "$SOURCE_CHANNEL_URL" ]]; then
...
if [[ "$SKIP_TRANSCRIPT" -ne 1 ]]; then
  TRANSCRIPT_ARGS=(--metadata "$METADATA_PATH")
  if [[ "$FORCE" -eq 1 ]]; then
    TRANSCRIPT_ARGS+=(--force)
  fi
  "$PYTHON_BIN" "$SCRIPT_DIR/generate_transcript.py" "${TRANSCRIPT_ARGS[@]}"
else
  METADATA_PATH_ENV="$METADATA_PATH" MANIFEST_PATH_ENV="$MANIFEST_PATH" "$PYTHON_BIN" - <<'PY'
import json
import os
from pathlib import Path

metadata_path = Path(os.environ["METADATA_PATH_ENV"])
manifest_path = Path(os.environ["MANIFEST_PATH_ENV"])
...
manifest["status"]["transcript"] = "ready"
manifest_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
PY
fi
...
if [[ -n "$SOURCE_PUBLIC_URL" && -n "$HOMEPAGE_PUBLISH_DATE" ]]; then
  ...
  if [[ "${RU_HOMEPAGE_DEPLOY:-0}" == "1" ]]; then
    (
      cd "$HOMEPAGE_SITE_DIR"
