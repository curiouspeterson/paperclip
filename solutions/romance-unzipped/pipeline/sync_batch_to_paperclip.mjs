...
function pickKeyAssets(manifest) {
  ...
  if (approvalPacketJsonPath && fs.existsSync(approvalPacketJsonPath)) {
    ...
    if (packet?.lead_asset?.preview_path) {
      leadAsset = path.resolve(packet.lead_asset.preview_path);
    }
    if (packet?.lead_asset?.quote_card_path) {
      quoteCard = path.resolve(packet.lead_asset.quote_card_path);
    }
  }
...
async function main() {
  ...
  const docSpecs = [
    ...
    },
    {
      ...
      changeSummary: "Synced generated Instagram dry-run packet from episode batch",
    },
    {
      key: "newsletter_dry_run",
      title: "Newsletter Dry Run",
      path: resolveIfPresent(newsletter.mailchimp_dry_run_path),
      intro: "# Newsletter Dry Run\n",
      changeSummary: "Synced generated newsletter dry-run packet from episode batch",
    },
    {
  ...
  const attachmentCandidates = Array.from(new Set([
    ...
    ...attachmentCandidatesFromManifest(manifest).slice(0, 8),
  ]));
