...
from pipeline_llm import ZAI_CODING_OPENAI_BASE_URL, chat_json, discover_llm_config, normalize_text
INSTAGRAM_HASHTAGS = [
    "#RomanceUnzipped",
    "#RomanceBooks",
    "#BookPodcast",
...
def llm_copy_bundle(context: dict, *, base_url: str, api_key: str, model: str, timeout_seconds: int) -> dict | None:
    ...
    for payload, max_tokens, temperature in attempts:
        parsed = chat_json(
            ...
            model=model,
            system_prompt=(
                "You write for Romance Unzipped. Respond with one JSON object only. "
                "No markdown fences, no analysis, no <think> tags."
            ),
...
def main() -> int:
    ...
    artifact_paths = {
        "transcript": Path(context["transcript_path"]).expanduser().resolve(),
        "clip candidates": Path(manifest["targets"]["clip_extractor"]["clip_candidates_json_path"]).expanduser().resolve(),
        "rendered clips": Path(manifest["targets"]["clip_extractor"]["rendered_clips_json_path"]).expanduser().resolve(),
        "quote cards": Path(manifest["targets"]["clip_extractor"]["quote_cards_json_path"]).expanduser().resolve(),
    }
    if not args.force and not validate_artifact_freshness(manifest, "approval_packet_inputs", artifact_paths):
        raise SystemExit(
            "Approval packet inputs are stale (upstream inputs changed). "
            "Re-run dependent scripts or use --force to regenerate."
        )
    ...
    record_artifact_provenance(manifest, "approval_packet_inputs", artifact_paths)

    status = manifest.setdefault("status", {})
    status["approval_packet"] = "ready"
    status["newsletter_draft"] = "ready"
    social_target["approval_packet_path"] = str(approval_packet_path)
    social_target["approval_packet_json_path"] = str(approval_packet_json_path)
    newsletter_target["draft_path"] = str(newsletter_draft_path)
    newsletter_target["draft_json_path"] = str(newsletter_draft_json_path)
